...
geojson_taluk_boundary_path = 'taluk_new.geojson'
traffic_accident_data_path = 'New Traffic Data (1).xlsx'
taluk_boundaries_gdf = gpd.read_file(geojson_taluk_boundary_path).to_crs("EPSG:4326")
traffic_data = pd.read_excel(traffic_accident_data_path)
...
traffic_accident_gdf = gpd.GeoDataFrame(
    traffic_data,
    geometry=gpd.points_from_xy(traffic_data['Longitude'], traffic_data['Latitude']),
    crs="EPSG:4326"
)
traffic_data_within_boundaries = gpd.sjoin(traffic_accident_gdf, taluk_boundaries_gdf, how="inner", predicate="within")
...
def predict():
   if request.method == "POST":
    try:
        ...
        if district in ["THIRUVANANTHAPURAM RURAL"]:
...
def list_districts():
    district_list = ["All"] + sorted(taluk_boundaries_gdf['DISTRICT'].dropna().unique().tolist())
    return jsonify(district_list)

@app.route("/list_taluks/<district>")
def list_taluks(district):
    if district == "All":
        taluk_list = ["All"] + sorted(taluk_boundaries_gdf['TALUK'].dropna().unique().tolist())
    else:
        taluk_list = ["All"] + sorted(taluk_boundaries_gdf[taluk_boundaries_gdf['DISTRICT'] == district]['TALUK'].dropna().unique().tolist())
    return jsonify(taluk_list)

@app.route("/generate_traffic_map", methods=["POST"])
def generate_traffic_map():
    district = request.json.get("district", "All")
    taluk_name = request.json.get("taluk", "All")

    # Filter data for selected district and taluk
    if taluk_name == "All":
        filtered_taluk_boundaries = taluk_boundaries_gdf
        filtered_traffic_data = traffic_data_within_boundaries
    else:
        filtered_taluk_boundaries = taluk_boundaries_gdf[taluk_boundaries_gdf['TALUK'] == taluk_name]
        filtered_traffic_data = traffic_data_within_boundaries[
            traffic_data_within_boundaries['TALUK'] == taluk_name
        ]

    # Generate the map
    traffic_map = folium.Map(location=[10.8505, 76.2711], zoom_start=7)
    if taluk_name != "All":
        folium.GeoJson(
            filtered_taluk_boundaries,
            style_function=lambda x: {'color': 'green', 'weight': 2, 'fillOpacity': 0.1}
        ).add_to(traffic_map)

    marker_cluster = MarkerCluster().add_to(traffic_map)
    for _, row in filtered_traffic_data.iterrows():
        folium.Marker(
            location=[row.geometry.y, row.geometry.x],
            popup=f"Location: {row.get('Place', 'Unknown')}<br>Latitude: {row.geometry.y}<br>Longitude: {row.geometry.x}"
        ).add_to(marker_cluster)

    # Save the map to an HTML file
    map_output_path = os.path.join("static", "traffic_map2.html")
    traffic_map.save(map_output_path)
    return jsonify({"success": True, "map_url": "traffic_map2.html"})

@app.route("/map")
def map_view():
    return render_template("map.html")  # Or any template you'd like to serve with the map
